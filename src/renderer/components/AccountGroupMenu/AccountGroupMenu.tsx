...
import styles from './AccountGroupMenu.module.scss';
import { Account } from '../Accounts/slice';
import ShowToast from '../Toast/Toast';

export default function AccountGroupMenu({
  hideMenu,
  saveData,
  menuData,
}: {
  hideMenu: (event: any) => void;
  saveData: (id: string, name: string, accounts: Account[]) => void;
  menuData: {
    id: string;
    name: string;
    accounts: Account[];
  ...
  const reduxData = useSelector((state: RootState) => {
    for (let i = 0; i < state.accounts.length; i += 1) {
      if (state.accounts[i].id === menuData.id) {
  ...
  const handleKeyPress = (event: any) => {
    if (event.key === 'Enter') {
      ...
      saveData(menuData.id, newName, reduxData.accounts);

      ShowToast({
        type: 'success',
        title: 'Account Group',
        message: 'Renamed account group!',
      });
    }
  };
